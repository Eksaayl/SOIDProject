...
class PartIIC extends StatefulWidget {
  ...
  const PartIIC({Key? key, required this.documentId}) : super(key: key);
...
class _PartIICState extends State<PartIIC> {
  ...
  Future<void> _save({bool finalize = false}) async {
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final docxRef = FirebaseStorage.instance
              .ref()
              .child('${_yearRange}/II.C/document.docx');
      ...
      if (finalize) {
        await createSubmissionNotification('Part II.C', _yearRange);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Part II.C submitted for admin approval. You will be notified once it is reviewed.'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Part II.C saved successfully (not finalized)'),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Are you sure you want to remove Database ${index + 1}? This action cannot be undone.',
                ...
                child: TextFormField(
                  ...
                  onChanged: (value) {
                    if (!value.startsWith(dbNumber)) {
                      final cleanValue = value.replaceAll(dbNumber, '').trim();
                      controllers['name_of_database']!.text = '$dbNumber. $cleanValue';
                      controllers['name_of_database']!.selection = TextSelection.fromPosition(
                        TextPosition(offset: controllers['name_of_database']!.text.length),
                      );
                    }
                  },
                ),
            ...
            TableRow(children: [
              Padding(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text('GENERAL CONTENTS/DESCRIPTION', style: TextStyle(fontWeight: FontWeight.bold)),
                    SizedBox(width: 4),
                    Tooltip(
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 4),
                    Tooltip(
                      message: 'Indicate the current status (e.g., Operational, Under Development, Planned).',
                      child: Icon(Icons.help_outline, size: 18, color: Colors.blueGrey),
                    ),
                  ],
                ),
            ...
            TableRow(children: [
              Padding(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text('INFORMATION SYSTEMS SERVED', style: TextStyle(fontWeight: FontWeight.bold)),
                    SizedBox(width: 4),
                    Tooltip(
                      message: 'List the information systems served by this database.',
                      child: Icon(Icons.help_outline, size: 18, color: Colors.blueGrey),
                    ),
                  ],
                ),
            ...
            TableRow(children: [
              Padding(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text('DATA ARCHIVING/STORAGE MEDIA', style: TextStyle(fontWeight: FontWeight.bold)),
                    SizedBox(width: 4),
                    Tooltip(
                      message: 'Describe the data archiving or storage media used.',
                      child: Icon(Icons.help_outline, size: 18, color: Colors.blueGrey),
                    ),
                  ],
                ),
