...
class _PartIAFormPageState extends State<PartIAFormPage> {
  ...
  void initState() {
    ...
    docNameCtrl.addListener(_markUnsaved);
    legalBasisCtrl.addListener(_markUnsaved);
    visionCtrl.addListener(_markUnsaved);
    missionCtrl.addListener(_markUnsaved);
    frameworkCtrl.addListener(_markUnsaved);
    for (final ctrl in pillarControllers) {
      ctrl.addListener(_markUnsaved);
    }
    for (final quill in functionControllers) {
      quill.addListener(_markUnsaved);
    }
  ...
  Future<void> _loadContent() async {
    try {
      ...
      if (data['pillar'] != null) {
        ...
        for (var pillar in pillars) {
          if (pillar.trim().isNotEmpty) {
            ...
            if (match != null) {
              final extractedText = match.group(1)?.trim() ?? '';
      ...
      if (pillarControllers.isEmpty) {
        pillarControllers.add(TextEditingController());
        pillarControllers.last.addListener(_markUnsaved);
      }
  ...
  Future<void> _save({bool finalize = false}) async {
    ...
    try {
      ...
      if (_uploadedDocxBytes != null) {
        ...
        final pillarText = pillarControllers
            .asMap()
            .entries
        ...
        final url = Uri.parse('${Config.serverUrl}/generate-ia-docx/');
      ...
      final payload = {
        ...
        'framework': frameworkCtrl.text.trim(),
        'visionStatement': visionCtrl.text.trim(),
        'missionStatement': missionCtrl.text.trim(),        'pillar': pillarText,
        'fileUrl': docxUrl,
        'modifiedBy': username,
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        if (_hasUnsavedChanges && !_isFinalized) {
          ...
          return Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                if (!_isFinalized && pillarControllers.length > 1)
                  Padding(
                    padding: const EdgeInsets.only(left: 8, top: 2),
                    child: IconButton(
                      icon: const Icon(Icons.delete, color: Colors.red),
                      tooltip: 'Delete Pillar',
                      onPressed: () async {
                        final confirmed = await DialogUtils.showDeleteConfirmationDialog(
                          context: context,
                          title: 'Delete Pillar',
                          message: 'Are you sure you want to delete Pillar ${idx + 1}? This action cannot be undone.',
                          confirmText: 'Delete Pillar',
                        );
                        if (confirmed == true) {
                          setState(() {
                            pillarControllers.removeAt(idx);
                          });
                        }
                      },
                    ),
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                setState(() {
                  ...
                  newController.addListener(_markUnsaved);
                  pillarControllers.add(newController);
                });
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xff021e84),
