...
class PartVB extends StatefulWidget {
  ...
  const PartVB({Key? key, this.documentId = 'document'}) : super(key: key);

  @override
  State<PartVB> createState() => _PartVBState();
}

class _PartVBState extends State<PartVB> {
  final _formKey = GlobalKey<FormState>();
  Uint8List? _isisBytes;
  bool _loading = true;
  bool _saving = false;
  ...
  Future<void> _loadContent() async {
    try {
      ...
      if (data != null) {
        ...
        try {
          final isisRef = _storage.ref().child('$_yearRange/V.B/isis.png');
          final isisBytes = await isisRef.getData();
          if (isisBytes != null) {
            setState(() {
              _isisBytes = isisBytes;
            });
          }
        } catch (e) {
          print('Error loading IS Implementation Schedule image: $e');
        }
      }
    } catch (e) {
  ...
  Future<void> _downloadImage() async {
    if (_isisBytes == null) return;

    try {
      if (kIsWeb) {
        await FileSaver.instance.saveFile(
          name: 'Part_V_B_$_yearRange.png',
          bytes: _isisBytes!,
          mimeType: MimeType.png,
        );
      } else {
        final directory = await getApplicationDocumentsDirectory();
        final file = File('${directory.path}/Part_V_B_$_yearRange.png');
        await file.writeAsBytes(_isisBytes!);
      }

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('IS Implementation Schedule image downloaded successfully'))
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<String?> _generateAndUploadDocx() async {
    try {
      if (_isisBytes == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload an IS Implementation Schedule image first'),
          ),
        );
        return null;
      }
  ...
  Future<void> _save({bool finalize = false}) async {
    ...
    try {
      ...
      if (finalize) {
        await createSubmissionNotification('Part V.B', _yearRange);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Part V.B submitted for admin approval. You will be notified once it is reviewed.'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Part V.B saved successfully (not finalized)'),
  ...
  Future<void> _downloadDocx() async {
    ...
    try {
      final fileName = 'Part_VB.docx';
      final storage = FirebaseStorage.instance;
      final docxRef = storage.ref().child('$_yearRange/V.B/document.docx');
        ...
        children: [
          Row(
            children: [
              ...
              const Text(
                'IS Implementation Schedule Image',
                style: TextStyle(
                  fontSize: 18,
          ...
          if (_isisBytes != null && !_isFinalized)
            const SizedBox(height: 20),
          if (_isisBytes != null && !_isFinalized)
            Center(
              child: ElevatedButton.icon(
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const Text(
                                'Please upload an IS Implementation Schedule image for Part V.B. The image will be inserted into the generated DOCX document using the placeholder "{ISIS}". Make sure the image is clear and properly formatted before finalizing. Click the document icon in the app bar to generate a DOCX file with the image.',
                                style: TextStyle(
                                  fontSize: 16,
